...
export function TestimonialCard({ testimonial, isActive }: TestimonialCardProps) {
  return (
    <div
      className={cn(
        "rounded-card p-7 border transition-all duration-500",
        isActive
    ...
    >
      ...
      <p className={cn(
        ...
        isActive ? "text-white/85" : "text-slate-muted"
      )}>
      ...
      <div className="flex items-center gap-3">
        ...
        />
        <div>
          <div className={cn("font-bold text-[0.9rem]", isActive ? "text-white" : "text-slate")}>
            {testimonial.name}
          </div>
