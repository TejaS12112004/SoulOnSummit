  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
      {stats.map((stat) => (
          <div
            key={stat.label}
            className="bg-card shadow-sm hover:shadow-md transition-shadow rounded-2xl p-5 flex flex-col items-center text-center gap-2.5 border border-border/40"
          >
            {stat.icon}
            <span className="text-muted-foreground text-[0.7rem] font-bold uppercase tracking-[0.15em]">
              {stat.label}
            </span>
            <span className="text-foreground font-semibold text-[0.95rem]">{stat.value}</span>
          </div>
      ))}
    </div>
  )
